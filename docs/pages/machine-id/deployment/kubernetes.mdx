...
<Tabs>
<TabItem name="Outputting SSH, Application or Database Credentials">
...
<TabItem name="Outputting Kubernetes credentials">

By default, when outputting Kubernetes credentials, `tbot` outputs make use of a Kubernetes exec
plugin to always provide the "latest" version of the credentials. When outputting Kubernetes credentials
to a Kubernetes secret, however, it is important to disable the use of the `exec` plugin so that a
static `kubeconfig` file is written instead:

```yaml
outputs:
  - type: kubernetes
    kubernetes_cluster: target-kube-cluster-name
