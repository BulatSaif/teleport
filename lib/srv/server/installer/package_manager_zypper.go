...
func (pm *packageManagerZypper) AddTeleportRepository(ctx context.Context, linuxInfo *linuxDistroInfo, repoChannel string) error {
	...
	pm.logger.InfoContext(ctx, "Trusting Teleport repository key", "command", "rpm --import "+aptPublicKeyEndpoint)
	importPublicKeyCMD := exec.CommandContext(ctx, pm.bins.rpm, "--import", aptPublicKeyEndpoint)
