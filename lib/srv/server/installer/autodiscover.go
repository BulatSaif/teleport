...
func (asi *AutodiscoverNodeInstaller) Install(ctx context.Context) error {
	...
	teleportBinaryLocation := asi.buildAbsoluteFilePath(defaultTeleportBinaryLocation)
	// Check if teleport is already installed.
	if _, err := os.Stat(teleportBinaryLocation); err == nil {
	...
	imdsClient, err := getIMDSClient(ctx)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	if asi.AutoUpgrades {
		...
		asi.Logger.InfoContext(ctx, "Auto-upgrade enabled: fetching target version")
	...
	asi.Logger.InfoContext(ctx, "Writing teleport configuration", "args", teleportNodeConfigureArgs)
	teleportNodeConfigureCmd := exec.CommandContext(ctx, teleportBinaryLocation, teleportNodeConfigureArgs...)
...
func getIMDSClient(ctx context.Context) (imds.Client, error) {
	...
	imdsClient, err := cloud.DiscoverInstanceMetadata(ctx, imdsProviders)
	if err != nil {
...
func (asi *AutodiscoverNodeInstaller) fetchTargetVersion(ctx context.Context) string {
	channelURL := "https://" + asi.ProxyPublicAddr + "/v1/webapi/automaticupgrades/channel/default"
	upgradeURL, err := url.Parse(channelURL)
	if err != nil {
...
func (l *linuxDistroInfo) packageManagerKind() packageManagerKind {
	...
	// TODO(marco): use l.IDLike field to infer distro (repo URL must use the IDLike value instead of ID in those cases)
